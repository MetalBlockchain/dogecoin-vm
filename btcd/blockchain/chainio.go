...
func (b *BlockChain) createChainState() error {
	...
	err := b.db.Update(func(dbTx database.Tx) error {
		...
		// consensus rules.
		// BTCVM MODIFICATION: We DO insert genesis outputs to make them
		// spendable, which differs from Bitcoin consensus rules but is
		// required for btcvm custom genesis blocks with premined coins.
		utxoBucket, err := meta.CreateBucket(utxoSetBucketName)
		if err != nil {
			return err
		}

		// BTCVM: Add genesis block outputs to UTXO set
		// This enables spending from genesis transactions, which is needed
		// for custom genesis blocks with premined coins.
		for txIdx, tx := range genesisBlock.MsgBlock().Transactions {
			for txOutIdx, txOut := range tx.TxOut {
				...
				// Create UTXO entry
				var flags txoFlags = tfModified | tfFresh
				if txIdx == 0 {
					// Mark as coinbase if it's the first transaction
					flags |= tfCoinBase
				}
...
// BlockByHashAny returns the block with the given hash from any chain (main or side)
// with the appropriate chain height set. Unlike BlockByHash, this does not require
// the block to be in the main chain, allowing retrieval of blocks on side chains or
// blocks that haven't been connected to the main chain yet.
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHashAny(hash *chainhash.Hash) (*btcutil.Block, error) {
	// Lookup the block hash in block index (checks ALL blocks)
	node := b.index.LookupNode(hash)
	if node == nil {
		return nil, fmt.Errorf("block %s is not known", hash)
	}

	// Verify the block data is actually stored (not just header-only)
	if !b.index.NodeStatus(node).HaveData() {
		return nil, fmt.Errorf("block %s header exists but data is not stored", hash)
	}
