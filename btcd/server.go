...
import (
	...
	"github.com/paulgnz/dogecoin-vm/btcd/mining/cpuminer"
...
type Server struct {
	...
	// OnTxRelay is a callback that is called when transactions are to be relayed.
	// This is used by the VM to gossip transactions via the Metal network.
	OnTxRelay func([]*mempool.TxDesc)

	// OnBlockRelay is a callback that is called when blocks are to be relayed.
	// This is used by the VM to gossip blocks via the Metal network.
	OnBlockRelay func(*btcutil.Block)

	// CustomProcessBlock is an optional callback for custom block processing.
	// If set, this will be called instead of the default blockchain processing.
	// This is useful for integrating with consensus mechanisms like Metal.
	CustomProcessBlock func(*btcutil.Block, blockchain.BehaviorFlags) (bool, error)
}

// CPUMiner returns the cpu miner instance.
...
func (s *Server) RelayInventory(invVect *wire.InvVect, data any) {
	// Use non-blocking send to avoid blocking blockchain notifications
	// when MaxPeers=0 (which makes relayInv channel unbuffered)
	go func() {
...
func newServer(
	...
	s.cpuMiner = cpuminer.New(&cpuminer.Config{
		...
		ProcessBlock:           s.processBlock, // Use direct blockchain processing
		ConnectedCount:         s.ConnectedCount,
		IsCurrent:              func() bool { return true }, // Always current for VM mode
	})
...
// TxMemPool returns the transaction memory pool.
func (s *Server) TxMemPool() *mempool.TxPool {
	return s.txMemPool
}
...
// handleBlockchainNotification handles blockchain events from the blockchain
// package and triggers appropriate actions (like gossiping blocks).
func (s *Server) handleBlockchainNotification(notification *blockchain.Notification) {
	switch notification.Type {
	case blockchain.NTBlockAccepted:
		block, ok := notification.Data.(*btcutil.Block)
		if !ok {
			srvrLog.Warnf("Block accepted notification has invalid data type")
			return
		}

		srvrLog.Debugf("Block accepted notification received: %s (height %d)",
			block.Hash(), block.Height())

		// If OnBlockRelay callback is set (by VM), use Metal gossip
		if s.OnBlockRelay != nil {
			s.OnBlockRelay(block)
			return
		}

		// Otherwise fall back to traditional Bitcoin P2P relay
		// (handled by existing netsync manager)

	case blockchain.NTBlockConnected:
		// Optional: Could add additional logic here
		srvrLog.Debugf("Block connected to main chain")

	case blockchain.NTBlockDisconnected:
		// Optional: Could add additional logic here
		srvrLog.Debugf("Block disconnected from main chain")
	}
}

// setupBlockchainNotifications subscribes to blockchain events
func (s *Server) setupBlockchainNotifications() {
	s.chain.Subscribe(s.handleBlockchainNotification)
	srvrLog.Info("Subscribed to blockchain notifications for block relay")
}

// processBlock is a wrapper around blockchain.ProcessBlock that matches the
// signature expected by the CPU miner. It bypasses the sync manager and
// processes blocks directly through the blockchain.
func (s *Server) processBlock(block *btcutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
	// If custom processing is set (e.g., from VM), use it
	if s.CustomProcessBlock != nil {
		return s.CustomProcessBlock(block, flags)
	}

	// Otherwise, call blockchain.ProcessBlock directly
	// Returns: (isMainChain, isOrphan, error)
	isMainChain, isOrphan, err := s.chain.ProcessBlock(block, flags)
	if err != nil {
		return false, err
	}

	srvrLog.Debugf("Processed block %s: isMainChain=%v, isOrphan=%v",
		block.Hash(), isMainChain, isOrphan)

	// Return isOrphan to match the expected signature
	return isOrphan, nil
}
